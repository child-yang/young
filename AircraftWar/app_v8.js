...
/*阶段4：游戏进行阶段*/
	...
	function Hero(imgs){//英雄构造方法
		...
		this.move=function(){//切换英雄当前状态
			...
			if(this.counter%5===0){//该数字可以做成变量，表示间隔多久发射一发子弹，根据等级来改变发射速度
				this.fire();
			}			
		}
	...
	function Bullet(img){//子弹的构造方法
		...
		this.move=function(){
			this.y-=10;//此处的数字可以做成变量，指定子弹移动的速度
	...
	function Enemy3(imgs){
		...
		this.height=imgs[0].height;
		this.index=0;//当前绘制的图片在数组中的下标
		this.speed=2;//大号敌机飞行速度
		this.removable=false;
		this.blood=7;//大号敌机只有5格血
		this.crashed=false;//是否被撞毁
		this.draw=function(){
			ctx.drawImage(imgs[this.index],this.x,this.y);
		}
...
/*阶段5：游戏暂停阶段*/
canvas.onmouseout = function(){	//鼠标移除事件
	if(curPhase===PHASE_PLAY){
		curPhase = PHASE_PAUSE;
	}
}
canvas.onmouseover = function(){	//鼠标移入事件
...
function drawPause(){
	...
	ctx.drawImage(imgGamePauseNor,x,x);
}
